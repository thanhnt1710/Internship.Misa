			...
			.done(function(res) {
				...
				$.each(field_input, function (index, input) {
					...
					if(id == undefined) {
						...
						//Xoa item duoc chon hien tai
						$(this).parent().siblings('.combobox__content').find(`.combobox__content__current`).removeClass('combobox__content__current');
						//Them item hien tai vao item dung nhu du lieu tra ve
						$(this).parent().siblings('.combobox__content').find(`div[valueId=${id}]`).addClass('combobox__content__current');
						//Lay ten dung nhu du lieu tra ve de gan cho input
						var content = $(this).parent().siblings('.combobox__content').find(`div[valueId=${id}] span`).text();
						$(this).attr('valueId', id);
						$(this).val(content);
					}
				});
			})
