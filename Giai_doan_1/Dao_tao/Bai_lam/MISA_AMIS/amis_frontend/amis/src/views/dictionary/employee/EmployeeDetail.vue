<template>
    <div class="modal"
        @keydown.ctrl.shift.83.exact.prevent.stop="submitAddForm"
        @keydown.ctrl.83.exact.prevent.stop="submitForm"
    > 
                            ...
                            <div class="modal-content-body-top-row-wrap">
                                <div class="form-item">
                                    ...
                                    <div class="form-item-content">
                                        ...
                                        <div v-if="!validateFormState.departmentName" class="tooltip-validate tooltip-validate-department">Vui lòng chọn 1 đơn vị cụ thể</div>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    watch: {
        "$store.state.popupDuplicate": function() {
            if(this.$store.state.popupDuplicate.status == false) {
    ...
    mounted() {
        ...
        //Hàm có thể gọi từ popup
        this.$root.$on('saveEmployee', () => {
            this.saveEmployee();
        });

        // //Hàm gọi lưu obj thông tin form lúc đầu vào store, dùng so sánh khi đóng form dữ liệu có sự thay đổi hay không
        // //param: commitEmployeeFormBefore: true -> dùng để commit lên store
        // //param:                           false -> build obj employee hiện tại trên form 
        // this.$root.$on('buildObjectForm', (commitEmployeeFormBefore) => {
        //     this.buildObjectForm(commitEmployeeFormBefore);
        // });
    },
    methods: {
                    ...
                    if(el.getAttribute('fieldValidate') == 'departmentName') {
                        ...
                        departmentCurrent.map(department => {
                            ...
                            if(value.trim() == department.DepartmentName.trim()) {//trùng
                                ...
                                //this.validateFormState[el.getAttribute("fieldValidate")] = true;
                            }
                        })
                        ...
                        if(!duplicateNameDepartment) {
                            ...
                            //this.validateFormState['departmentName'] = false;
                        }
                    }
                    //2 trường còn lại
                    else {
                        if(value == '') {//Nếu k có giá trị nhập vào
                            ...
                            //this.validateFormState[el.getAttribute("fieldValidate")] = false;
                        }
                        else {
                            //Ẩn border đỏ
                            el.removeAttribute("style");
        ...
        buildObjectForm() {
            ...
            if(this.$store.state.formMode == Const.FORM_MODE_EDIT) {
        ...
        saveEmployee() {
            ...
            if(validateForm) {//Nếu validate thành công ms thực hiện
                let employee = this.buildObjectForm(false);//gọi hàm build dữ liệu thành obj

                //Thông tin form chi tiết gửi cho store để lưu dữ liệu
                let formInfor = {
                    employee: employee,
                    mode: Const.FORM_SAVED,//Cất 
                }

                //Goi Server thuc hien luu du lieu
                //Trước khi gọi, kiểm tra gọi để thêm mới (Add) hay nhân bản (Add) hay sưa (Edit) 
                if(this.$store.state.formMode == Const.FORM_MODE_ADD || this.$store.state.formMode == Const.FORM_MODE_REPLICA) {// Neu la them moi hoặc nhân bản -> POST
                    this.$store.dispatch('addEmployee', formInfor);
                }
                
                // neu la sua -> PUT
                if(this.$store.state.formMode == Const.FORM_MODE_EDIT) {// Neu la them moi -> PUT
        ...
        saveAndAddEmployee() {
            ...
            let validateForm = this.validateForm();
            if(validateForm) {//Nếu validate thành công ms thực hiện
                let employee = this.buildObjectForm(false);//gọi hàm build lữ liệu thành obj
    
                //Thông tin form chi tiết gửi cho store để lưu dữ liệu
                let formInfor = {
                    employee: employee,
                    mode: Const.FORM_SAVED_ADD,//Cất và thêm 
                }
                ...
                if(this.$store.state.formMode == Const.FORM_MODE_ADD || this.$store.state.formMode == Const.FORM_MODE_REPLICA) {// Neu la them moi hoặc nhân bản -> POST
                    this.$store.dispatch('addEmployee', formInfor);
                    //Gọi hàm build dữ liệu form mới và lưu vào store
                    setTimeout(()=>{
                        this.buildObjectForm(true);
                    }, 1000);
                }
        ...
        selectItemCombobox(e,departmentId) {
            ...
            //this.validateFormState['departmentName'] = true;
        },
        ...
        handleCancelForm() {
            ...
            //Nếu không có thay đổi -> đóng form luôn
