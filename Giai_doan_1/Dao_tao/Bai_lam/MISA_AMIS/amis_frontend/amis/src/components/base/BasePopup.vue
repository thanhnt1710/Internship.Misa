    ...
    export default {
        ...
        methods: {
            ...
            cancelPopupValidate() {
                ...
                //Khi ấn đóng focus vào ô bị lỗi, thông tin lỗi lấy từ store
                let arrayValidateFormState = Object.entries(this.$store.state.validateFormState);
                //Duyệt mảng
                arrayValidateFormState.map(validateFormState => {
                    if(!validateFormState[1]) {//Nếu trường hiện tại validate sai -> focus
                        document.getElementById(validateFormState[0]).focus();
                    }
                })
            },
